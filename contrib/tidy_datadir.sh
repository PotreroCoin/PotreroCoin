...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Potrero database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Potrero datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Potrero datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Potrero 0.7 datadir."
    ;;
  3)
    echo "Detected Potrero pre-0.8 datadir."
    ;;
  4)
    echo "Detected Potrero 0.8 datadir."
    ;;
esac
